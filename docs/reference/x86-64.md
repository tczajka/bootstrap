...
Increments a register or memory location described by the
[ModRM byte](#memory-addressing-modrm-byte) by 1.
...
Decrements a register or memory location described by the
[ModRM byte](#memory-addressing-modrm-byte) by 1.
...
Negate a signed register or memory location described by the
[ModRM byte](#memory-addressing-modrm-byte).
...
Bitwise negation of a register or memory location described by the
[ModRM byte](#memory-addressing-modrm-byte).
